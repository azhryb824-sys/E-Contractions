...
function loadItemDict() {
  try {
    ...
    const dict = {};
    if (Array.isArray(items)) { for (const i of items) { if (i && i.code) dict[i.code] = i; } }
    return dict;
  } catch (e) { return {}; }
}
const itemDictionary = loadItemDict();

class Decimal {
  constructor(value) {
    this.value = parseFloat(value) || 0;
  }
  add(other) { return new Decimal(this.value + (other instanceof Decimal ? other.value : other)); }
  sub(other) { return new Decimal(this.value - (other instanceof Decimal ? other.value : other)); }
  mul(other) { return new Decimal(this.value * (other instanceof Decimal ? other.value : other)); }
  div(other) { const v = other instanceof Decimal ? other.value : other; if (v === 0) return new Decimal(0); return new Decimal(this.value / v); }
  round(decimals) { const f = Math.pow(10, decimals); return new Decimal(Math.round(this.value * f) / f); }
...
/**
 * Calculate the total cost for a single line item using Decimal arithmetic.
 *
 * @param {number} quantity - quantity of the item
 * @param {number} unitPrice - price per unit
 * @param {number} [wasteRate=0.05] - waste factor (e.g. 0.05 = 5%)
 * @param {number} [profitMargin=0.20] - profit margin (e.g. 0.20 = 20%)
 * @param {number} [taxRate=0.15] - tax rate (e.g. 0.15 = 15%)
 * @returns {{ quantity: number, unitPrice: number, wasteRate: number, quantityWithWaste: number, directCost: number, overhead: number, profit: number, tax: number, total: number, breakdown: Array<{item: string, value: *}> }}
 *
 * @example
 * calculateItemTotal(100, 50, 0.05, 0.20, 0.15)
 * // => { quantity: 100, unitPrice: 50, wasteRate: 0.05, quantityWithWaste: 105,
 * //      directCost: 5250, overhead: 525, profit: 1155, tax: 1039.5, total: 7969.5, ... }
 */
function calculateItemTotal(quantity, unitPrice, wasteRate = 0.05, profitMargin = 0.20, taxRate = 0.15) {
  const q = new Decimal(quantity);
  const p = new Decimal(unitPrice);
  const w = new Decimal(wasteRate);
  const m = new Decimal(profitMargin);
  const t = new Decimal(taxRate);

  const qWithWaste = q.mul(new Decimal(1).add(w));
  const directCost = qWithWaste.mul(p);
  const overhead = directCost.mul(new Decimal(0.10));
  const subTotal = directCost.add(overhead);
  const profit = subTotal.mul(m);
  const beforeTax = subTotal.add(profit);
  const tax = beforeTax.mul(t);
  const total = beforeTax.add(tax);

  return {
    quantity: q.toNumber(),
    unitPrice: p.toNumber(),
    wasteRate: w.toNumber(),
    quantityWithWaste: qWithWaste.round(2).toNumber(),
    directCost: directCost.round(2).toNumber(),
    overhead: overhead.round(2).toNumber(),
    profit: profit.round(2).toNumber(),
    tax: tax.round(2).toNumber(),
    total: total.round(2).toNumber(),
    breakdown: [
      { item: 'الكمية', value: q.toNumber() },
      { item: 'سعر الوحدة', value: p.toNumber() },
      { item: 'نسبة الهالك', value: `${(w.toNumber() * 100).toFixed(1)}%` },
      { item: 'الكمية بعد الهالك', value: qWithWaste.round(2).toNumber() },
      { item: 'التكلفة المباشرة', value: directCost.round(2).toNumber() },
      { item: 'المصروفات (10%)', value: overhead.round(2).toNumber() },
      { item: 'الربح', value: profit.round(2).toNumber() },
      { item: 'الضريبة', value: tax.round(2).toNumber() },
      { item: 'الإجمالي', value: total.round(2).toNumber() }
    ]
  };
}

/**
 * Calculate the total for a section (array of items) using Decimal arithmetic.
 *
 * @param {Array<{quantity: number, unitPrice: number}>} items
 * @param {number} [profitMargin=0.20]
 * @param {number} [taxRate=0.15]
 * @returns {{ items: Array, subtotal: number, overhead: number, profit: number, tax: number, total: number }}
 *
 * @example
 * calculateSectionTotal([{quantity: 10, unitPrice: 100}, {quantity: 5, unitPrice: 200}])
 * // => { subtotal: 2000, overhead: 200, profit: 440, tax: 396, total: 3036 }
 */
function calculateSectionTotal(items, profitMargin = 0.20, taxRate = 0.15) {
  const results = items.map(item => calculateItemTotal(item.quantity, item.unitPrice, item.wasteRate, profitMargin, taxRate));
  const subtotal = results.reduce((s, r) => s.add(new Decimal(r.directCost)), new Decimal(0));
  const overhead = results.reduce((s, r) => s.add(new Decimal(r.overhead)), new Decimal(0));
  const profit = results.reduce((s, r) => s.add(new Decimal(r.profit)), new Decimal(0));
  const tax = results.reduce((s, r) => s.add(new Decimal(r.tax)), new Decimal(0));
  const total = results.reduce((s, r) => s.add(new Decimal(r.total)), new Decimal(0));
...
/**
 * Calculate the grand total across multiple sections using Decimal arithmetic.
 *
 * @param {Array<{name: string, items: Array}>} sections
 * @param {number} [profitMargin=0.20]
 * @param {number} [taxRate=0.15]
 * @returns {{ sections: Array, grandTotal: number }}
 *
 * @example
 * calculateProjectTotal([{name: 'section1', items: [...]}, {name: 'section2', items: [...]}])
 * // => { sections: [...], grandTotal: 12345.67 }
 */
function calculateProjectTotal(sections, profitMargin = 0.20, taxRate = 0.15) {
  const sectionResults = sections.map(s => ({
    name: s.name,
    ...calculateSectionTotal(s.items, profitMargin, taxRate)
  }));
  const grandTotal = sectionResults.reduce((s, r) => s.add(new Decimal(r.total)), new Decimal(0));

  return {
    sections: sectionResults,
    grandTotal: grandTotal.round(2).toNumber()
  };
}

/**
 * Calculate quantity including waste using Decimal arithmetic.
 *
 * @param {number} quantity - base quantity
 * @param {number} wasteRate - waste rate (e.g. 0.05 for 5%)
 * @returns {number} quantity including waste, rounded to 2 decimals
 *
 * @example
 * calculateWithWaste(100, 0.05) // => 105
 */
function calculateWithWaste(quantity, wasteRate) {
  const q = new Decimal(quantity);
  const w = new Decimal(wasteRate);
  return q.mul(new Decimal(1).add(w)).round(2).toNumber();
}

/**
 * Calculate labor cost using Decimal arithmetic.
 *
 * @param {number} quantity - units of work
 * @param {number} laborRatePerUnit - labor cost per unit
 * @returns {number} total labor cost, rounded to 2 decimals
 *
 * @example
 * calculateLaborCost(100, 50) // => 5000
 */
function calculateLaborCost(quantity, laborRatePerUnit) {
  const q = new Decimal(quantity);
  const r = new Decimal(laborRatePerUnit);
  return q.mul(r).round(2).toNumber();
}

/**
 * Calculate material cost including waste using Decimal arithmetic.
 *
 * @param {number} quantity - base material quantity
 * @param {number} materialRatePerUnit - cost per unit of material
 * @param {number} [wasteRate=0.05] - waste factor (e.g. 0.05 for 5%)
 * @returns {number} total material cost, rounded to 2 decimals
 *
 * @example
 * calculateMaterialCost(100, 30, 0.05) // => 3150
 */
function calculateMaterialCost(quantity, materialRatePerUnit, wasteRate = 0.05) {
  const q = new Decimal(quantity);
  const r = new Decimal(materialRatePerUnit);
  const w = new Decimal(wasteRate);
  return q.mul(new Decimal(1).add(w)).mul(r).round(2).toNumber();
}

/**
 * Calculate transport cost using Decimal arithmetic.
 *
 * @param {number} distance - distance in km
 * @param {number} weight - weight in tons/kg
 * @param {number} ratePerKm - cost per km per unit weight
 * @returns {number} total transport cost, rounded to 2 decimals
 *
 * @example
 * calculateTransportCost(100, 5, 2) // => 1000
 */
function calculateTransportCost(distance, weight, ratePerKm) {
  const d = new Decimal(distance);
  const w = new Decimal(weight);
  const r = new Decimal(ratePerKm);
  return d.mul(w).mul(r).round(2).toNumber();
}

/**
 * Calculate overhead from direct costs using Decimal arithmetic.
 *
 * @param {number} directCosts - sum of direct costs
 * @param {number} overheadRate - overhead rate (e.g. 0.10 for 10%)
 * @returns {number} overhead amount, rounded to 2 decimals
 *
 * @example
 * calculateOverhead(10000, 0.10) // => 1000
 */
function calculateOverhead(directCosts, overheadRate) {
  const d = new Decimal(directCosts);
  const r = new Decimal(overheadRate);
  return d.mul(r).round(2).toNumber();
}

/**
 * Calculate profit on direct costs + overhead using Decimal arithmetic.
 *
 * @param {number} directPlusOverhead - direct costs + overhead
 * @param {number} profitMargin - profit margin (e.g. 0.20 for 20%)
 * @returns {number} profit amount, rounded to 2 decimals
 *
 * @example
 * calculateProfit(11000, 0.20) // => 2200
 */
function calculateProfit(directPlusOverhead, profitMargin) {
  const d = new Decimal(directPlusOverhead);
  const m = new Decimal(profitMargin);
  return d.mul(m).round(2).toNumber();
}

/**
 * Calculate tax on total before tax using Decimal arithmetic.
 *
 * @param {number} totalBeforeTax - total before adding tax
 * @param {number} taxRate - tax rate (e.g. 0.15 for 15%)
 * @returns {number} tax amount, rounded to 2 decimals
 *
 * @example
 * calculateTax(13200, 0.15) // => 1980
 */
function calculateTax(totalBeforeTax, taxRate) {
  const t = new Decimal(totalBeforeTax);
  const r = new Decimal(taxRate);
  return t.mul(r).round(2).toNumber();
}

/**
 * Calculate the grand total from an items array using Decimal arithmetic.
 *
 * Each item may have: quantity, unitPrice, wasteRate (default 0.05),
 * profitMargin (default 0.20), taxRate (default 0.15).
 *
 * @param {Array<{quantity: number, unitPrice: number, wasteRate?: number, profitMargin?: number, taxRate?: number}>} items
 * @param {{profitMargin?: number, taxRate?: number}} [config] - optional global defaults
 * @returns {{ items: Array, subtotal: number, overhead: number, profit: number, tax: number, total: number }}
 *
 * @example
 * calculateGrandTotal([{quantity: 10, unitPrice: 100}, {quantity: 5, unitPrice: 200}])
 * // => { items: [...], subtotal: 2000, overhead: 200, profit: 440, tax: 396, total: 3036 }
 */
function calculateGrandTotal(items, config = {}) {
  const globalProfitMargin = config.profitMargin !== undefined ? config.profitMargin : 0.20;
  const globalTaxRate = config.taxRate !== undefined ? config.taxRate : 0.15;

  const results = items.map(item => calculateItemTotal(
    item.quantity,
    item.unitPrice,
    item.wasteRate !== undefined ? item.wasteRate : 0.05,
    item.profitMargin !== undefined ? item.profitMargin : globalProfitMargin,
    item.taxRate !== undefined ? item.taxRate : globalTaxRate
  ));

...
function assertNonEmpty(data) {
  if (!data || typeof data !== 'object') {
    throw new Error('فشل إنشاء الملف: البيانات غير موجودة');
  }
  const sections = data.sections || [];
  const itemCount = sections.reduce((sum, s) => sum + (s.items || []).length, 0);
  if (itemCount === 0) {
    throw new Error('فشل إنشاء الملف: لم يتم إنشاء أي بند لجدول الكميات.');
  }
  if (data.document_type === 'quotation' && !data.sections?.length) {
    throw new Error('تعذر إنشاء عرض السعر: بيانات جدول الكميات غير موجودة.');
  }
  return true;
}
